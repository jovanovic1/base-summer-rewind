...
var BUILTIN_NAME = '__';
var isFieldNullable = (schema, typename, fieldName) => {
  var field = getField(schema, typename, fieldName);
  return !!field && field.type.kind !== 'NON_NULL';
};
var isListNullable = (schema, typename, fieldName) => {
  var field = getField(schema, typename, fieldName);
  if (!field) return false;
  var ofType = field.type.kind === 'NON_NULL' ? field.type.ofType : field.type;
  return ofType.kind === 'LIST' && ofType.ofType.kind !== 'NON_NULL';
};
var isFieldAvailableOnType = (schema, typename, fieldName) => fieldName.indexOf(BUILTIN_NAME) === 0 || typename.indexOf(BUILTIN_NAME) === 0 || !!getField(schema, typename, fieldName);
...
var getField = (schema, typename, fieldName) => {
...
function expectValidKeyingConfig(schema, keys) {
  if (process.env.NODE_ENV !== 'production') {
    for (var key in keys) {
      if (process.env.NODE_ENV !== 'production') {
        if (!schema.types.has(key)) {
          warn('Invalid Object type: The type `' + key + '` is not an object in the defined schema, but the `keys` option is referencing it.', 20);
        }
      }
    }
  }
}
function expectValidUpdatesConfig(schema, updates) {
  if (process.env.NODE_ENV === 'production') {
    return;
  }
  for (var typename in updates) {
    if (!updates[typename]) {
      ...
      return process.env.NODE_ENV !== 'production' ? warn('Invalid updates type: The type `' + typename + '` is not an object in the defined schema, but the `updates` config is referencing it.' + addition, 21) : void 0;
    }
    var fields = schema.types.get(typename).fields();
    for (var fieldName in updates[typename]) {
      if (process.env.NODE_ENV !== 'production') {
        if (!fields[fieldName]) {
          warn('Invalid updates field: `' + fieldName + '` on `' + typename + '` is not in the defined schema, but the `updates` config is referencing it.', 22);
        }
      }
    }
  }
}
function warnAboutResolver(name) {
  process.env.NODE_ENV !== 'production' ? warn(`Invalid resolver: \`${name}\` is not in the defined schema, but the \`resolvers\` option is referencing it.`, 23) : void 0;
}
function warnAboutAbstractResolver(name, kind) {
  process.env.NODE_ENV !== 'production' ? warn(`Invalid resolver: \`${name}\` does not match to a concrete type in the schema, but the \`resolvers\` option is referencing it. Implement the resolver for the types that ${kind === 'UNION' ? 'make up the union' : 'implement the interface'} instead.`, 26) : void 0;
}
function expectValidResolversConfig(schema, resolvers) {
  if (process.env.NODE_ENV === 'production') {
    return;
  }
  for (var key in resolvers) {
    if (key === 'Query') {
      ...
      if (!schema.types.has(key)) {
        ...
        warnAboutAbstractResolver(key, schema.types.get(key).kind);
      } else {
...
function expectValidOptimisticMutationsConfig(schema, optimisticMutations) {
  if (process.env.NODE_ENV === 'production') {
    return;
  }
  if (schema.mutation) {
    ...
    for (var mutation in optimisticMutations) {
      if (process.env.NODE_ENV !== 'production') {
        if (!validMutations[mutation]) {
          warn(`Invalid optimistic mutation field: \`${mutation}\` is not a mutation field in the defined schema, but the \`optimistic\` option is referencing it.`, 24);
        }
      }
    }
  }
...
var isFragmentHeuristicallyMatching = (node, typename, entityKey, vars) => {
  if (!typename) return false;
  var typeCondition = getTypeCondition(node);
...
var makeSelectionIterator = (typename, entityKey, defer, selectionSet, ctx) => {
  var child;
  var index = 0;
    ...
    while (child || index < selectionSet.length) {
      ...
      if (child) {
        ...
        if (!shouldInclude(select, ctx.variables)) ; else if (select.kind !== graphql_web.Kind.FIELD) {
          ...
          if (fragment) {
            var isMatching = !fragment.typeCondition || (ctx.store.schema ? isInterfaceOfType(ctx.store.schema, fragment, typename) : isFragmentHeuristicallyMatching(fragment, typename, entityKey, ctx.variables));
            if (isMatching) {
              if (process.env.NODE_ENV !== 'production') pushDebugNode(typename, fragment);
              child = makeSelectionIterator(typename, entityKey, defer || isDeferred(select, ctx.variables), getSelectionSet(fragment), ctx);
            }
          }
...
var ensureLink = (store, ref) => {
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (!link && ref && typeof ref === 'object') {
      warn("Can't generate a key for link(...) item." + '\nYou have to pass an `id` or `_id` field or create a custom `keys` config for `' + ref.__typename + '`.', 12);
    }
  }
  return link;
};
...
var readRoot = (ctx, entityKey, select, input) => {
  ...
  var iterate = makeSelectionIterator(entityKey, entityKey, deferRef, select, ctx);
  var node;
...
function getFieldResolver(directives, typename, fieldName, ctx) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (fieldResolver && directiveResolver) {
      warn(`A resolver and directive is being used at "${typename}.${fieldName}" simultaneously. Only the directive will apply.`, 28);
    }
  }
...
var readSelection = (ctx, key, select, input, result) => {
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (!isQuery && !!ctx.store.rootNames[entityKey]) {
      warn('Invalid root traversal: A selection was being read on `' + entityKey + '` which is an uncached root type.\n' + 'The `' + ctx.store.rootFields.mutation + '` and `' + ctx.store.rootFields.subscription + '` types are special ' + 'Operation Root Types and cannot be read back from the cache.', 25);
    }
  }
  ...
  var iterate = makeSelectionIterator(typename, entityKey, deferRef, select, ctx);
  var hasFields = false;
  var hasNext = false;
  var hasChanged = currentForeignData;
  var node;
  ...
  while ((node = iterate()) !== undefined) {
    ...
    if (fieldName === '__typename') {
      ...
      if (store.schema && dataFieldValue === null && !isFieldNullable(store.schema, typename, fieldName)) {
    ...
    if (!deferRef && dataFieldValue === undefined && (directives.optional || !!getFieldError(ctx) || store.schema && isFieldNullable(store.schema, typename, fieldName))) {
...
var resolveResolverResult = (ctx, typename, fieldName, key, select, prevData, result, isOwnedData) => {
  if (Array.isArray(result)) {
    ...
    var _isListNullable = store.schema ? isListNullable(store.schema, typename, fieldName) : false;
    var hasPartials = ctx.partial;
...
var writeSelection = (ctx, entityKey, select, data) => {
  ...
  if (!typename) {
    process.env.NODE_ENV !== 'production' ? warn("Couldn't find __typename when writing.\n" + "If you're writing to the cache manually have to pass a `__typename` property on each entity in your data.", 14) : void 0;
    return;
  } else if (!isRoot && entityKey) {
    writeRecord(entityKey, '__typename', typename);
  }
  var updates = ctx.store.updates[typename];
  var iterate = makeSelectionIterator(typename, entityKey || typename, deferRef, select, ctx);
  var node;
  while (node = iterate()) {
    var fieldName = getName(node);
    var fieldArgs = getFieldArguments(node, ctx.variables);
    ...
    if (fieldValue === undefined) {
      if (process.env.NODE_ENV !== 'production') {
        if (!entityKey || !hasField(entityKey, fieldKey) || ctx.optimistic && !readRecord(entityKey, '__typename')) {
          ...
          process.env.NODE_ENV !== 'production' ? warn('Invalid undefined: The field at `' + fieldKey + '` is `undefined`, but the GraphQL query expects a ' + expected + ' for this field.', 13) : void 0;
        }
      }
...
class Store {
  constructor(opts) {
    ...
    if (this.schema && process.env.NODE_ENV !== 'production') {
      ...
      expectValidOptimisticMutationsConfig(this.schema, this.optimisticMutations);
    }
  }
  ...
  resolve(entity, field, args) {
    ...
    if (entityKey) {
      ...
      fieldValue = readRecord(entityKey, fieldKey);
      if (fieldValue === undefined) fieldValue = readLink(entityKey, fieldKey);
    }
    return fieldValue;
  }
